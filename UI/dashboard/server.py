...
class HeraclitusDashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/evaluate":
            ...
            result = subprocess.run([compiler_path, project_dir], capture_output=True, text=True, cwd=repo_root)
            ...
            for line in lines:
                ...
                if "SVE-L301" in token or "SVE-L101" in token or "SVE-L201" in token or "SVE-L122" in token:
                    quarantine_count += 1
                    ...
                    trace_records.append({
                        ...
                        "clause": f"Logical Disjunction Fraud isolated. polarized binary choice constraint flagged."
                    })
                    ...
                    trace_records.append({
                        ...
                        "clause": f"Ontological Discrepancy flagged: {token}"
                    })
                elif "SVE-L402" in token or "LEAN4" in token or "MATH_FAILED" in token or "SVE-L121" in token:
                    quarantine_count += 1
                    phenomenology_count += 1
                    trace_records.append({
                        "index": chunk_id, "line": line_num, "pillar": "phenomenology",
                        "status": "quarantine", "lemma_id": token, "environment": "Observational Metric Core",
                        "clause": f"Empirical Failure Intercepted: {token}"
                    })
                else:
                    clean_count += 1
                    epistemology_count += 1

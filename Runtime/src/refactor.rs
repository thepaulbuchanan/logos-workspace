...
impl LemmaRefactor {
    // 🔒 CRYPTO SYNCHRONISED HASHER: Accepts all 4 structural parameters natively
    pub fn calculate_hash(id: &str, name: &str, triggers: &[String], body: &str) -> String {
        let mut hash: usize = 5381;
        for c in id.bytes() { hash = ((hash << 5).wrapping_add(hash)) + c as usize; }
        for c in name.bytes() { hash = ((hash << 5).wrapping_add(hash)) + c as usize; }
        for tr in triggers {
            for b in tr.bytes() { hash = ((hash << 5).wrapping_add(hash)) + b as usize; }
        }
        for b in body.bytes() { hash = ((hash << 5).wrapping_add(hash)) + b as usize; }
